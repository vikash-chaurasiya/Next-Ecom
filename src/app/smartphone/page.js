...
import { useGetAllProductQuery, useGetProductByCategoryQuery } from "@/toolkit/apiSlice";
import { useState } from "react";
import { shuffleArray } from "@/utils/commonFunc";
import CardLoading from "@/components/common/loading";

const Smartphone = () => {
  const res = useGetProductByCategoryQuery("smartphones");
  ...
  return (
    <>
      <main className="bg-slate-100 pt-5  px-24">
        {res.isLoading ? (
          <CardLoading />
        ) : !res.isSuccess ?  "Page Error" : (
         ...
         <section className="pb-5">
          ...
          {allProductRes.isLoading ? (
            <CardLoading/>
          ) : !allProductRes.isSuccess ? "Page Error" : (
            <div className="grid grid-cols-4  gap-10">
